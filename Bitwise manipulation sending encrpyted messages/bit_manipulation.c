...
//method to check if a bit in a char is set.
int isCharBitSet(char c, int bitNum)

{
    //if the position at bitNum is set then return 1.
    if(c &1<<bitNum)
    {
    	return 1;
    }
    
    //return 0 if the position at bitNum is not set.
    else
    {
    	return 0;
    }
...
//method to check if a bit in a short is set
...
//method to set specific bit to 1 in short.
void setShortBit(int bitNum, short *num)

{
    //will set a specific bit in the short. (change from 0 to 1)
    *num = *num|(1<<bitNum);	
...
//method to set specific bit to 1 in char.
void setCharBit(int bitNum, char *c)

{

    //will set a specific bit in the char. (change from 0 to 1)
    *c = *c|(1<<bitNum);
}
...
//count the number of bits set to 1 in a short
int countBits(short num)

{ 
    //initialzing count to 0
    unsigned int count = 0;
    
    //while loop to keep adding to count until bit size is reached.
    while(num)
    {
    	//adding 1 to count for each bit set 
    	count += num & 1;
    	
    	//right shift to traverse num
    	num>>=1;
...
//flips a specific bit position in num from 1 to 0 or 0 to 1.
void flipBitShort(int bitNum, short *num)//2 0100

{
     //if the bit at a specific position is set to 1, set it to 0.
     if(isShortBitSet(*num, bitNum) == 1)
     {
     	*num = *num&~(1<<bitNum);
     }
     
     //if the bit at a specific position is set to 0, set it to 1.
     else
     {
