...
int setParityHelper(int mask, int sum, short *num, int parityPos);
...

{
	...
	int sum;
    	...
    	//calling helper function to set parity bit P1
	setParityHelper(P1_MASK, sum, num, 1);
	
	//calling helper function to set parity bit P2
	setParityHelper(P2_MASK, sum, num, 2);

	//calling helper function to set parity bit P4
	setParityHelper(P4_MASK, sum, num, 4);

	//calling helper function to set parity bit P8
	setParityHelper(P8_MASK, sum, num, 8);


	return(0);
}

//setParityBits helper function.
int setParityHelper(int mask, int sum, short *num, int parityPos)
{
	...
	//count the number of bits that are set to 1
   	//if the number of bits is odd then set parity bit
	if(countBits(sum) % 2 != 0) 
	{
		//setting specific bit to 1
		setShortBit(parityPos, num);
	}
}
